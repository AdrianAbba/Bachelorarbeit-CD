...
Projects.memberFieldsNoGrading = {
  _id: 1,
  createdAt: 1,
};
...
if (Meteor.isServer) {
  ...
  Meteor.publish('supervisorProjects', function superVisorProjectsPublication(userId) {
    new SimpleSchema({
      userId: {
        type: String,
        regEx: SimpleSchema.RegEx.Id,
    ...
    return Projects.find({ supervisors: { $elemMatch: {userId: userId } } }, {
      fields: Projects.memberFields,
    });
  });
  //***************Beginn Coursefeature****************
...
export const ProjectsIndex = new Index({
  collection: Projects,
  fields: ['title', 'subtitle', 'description', 'jobs.joblabel', 'tags', 'occasions'],
  ...
  engine: new ElasticSearchEngine({
    ...
    client: { host: 'http://elastic:changeme@dev.projektor.mt.haw-hamburg.de:9200', log: 'trace' },
    query: (searchObject) => {
      if (searchObject.title) {
        const finalQuery = { bool: { must: [] } };
        const words = searchObject.title;
